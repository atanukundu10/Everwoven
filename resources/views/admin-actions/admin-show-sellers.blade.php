...
<body>
    ...
    @if(isset($all_sellers))
    <div class="table-container">
        <h3 class="text-center mb-4">All Seller Details</h3>
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-hover align-middle">
                ...
                <tbody>
                    @foreach($all_sellers->all() as $all)
                    <tr>
                        <td>{{ $all->seller_id }}</td>
                        <td>{{ $all->fullname }}</td>
                        <td>{{ $all->email }}</td>
                        <td>{{ $all->gstin }}</td>
                        <td>
                            <a href="{{url('/admin-delete-seller')}}{{$all->seller_id}}" class="btn btn-sm btn-danger mb-1">
                                <i class="fas fa-trash-alt me-1"></i>Delete
                            </a>
                            @if($all->status == 'unblocked')
                            <a href="{{url('/admin-block-seller')}}{{$all->seller_id}}" class="btn btn-sm btn-warning mb-1">
                                <i class="fas fa-ban me-1"></i>Block
                            </a>
                            @else
                            <a href="{{url('/admin-unblock-seller')}}{{$all->seller_id}}" class="btn btn-sm btn-success mb-1">
